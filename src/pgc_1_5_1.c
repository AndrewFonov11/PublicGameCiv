// Public Game Civ version 1.5.1
// Released into the public domain under CC0

#include <stdio.h>
#include <stdlib.h>

int ath = 1,year = 0 - 1500,tp = 0,rnd,tl = 1,res = 10,rpls = 1,rg = 1,ep = 1,defn = 0,alnc = 0,city = 0,win,pr,tc = 0,seed;
char v;
char civ_name[20];
...
void war()
{
	...
	if (win == 1)
	{
		...
		if (rnd == 1)
		{
			printf("\nThe army failed to break through the defense");
		}
		else
		{
			res += rand() % 5 + 1;
	...
	else if (win == 2)
	{
		...
		{
			rnd = rand() % 3 + 1;
			printf("\nPaying tribute to the enemy - %d",rnd);
			rpls -= rnd;
		}
		rnd = rand() % 2 + 1;
		if ((rnd == 1) && (defn == 1))
...
void random_()
{
	...
	else if (rnd == 2)
	{
		...
		if (ep == 1) ath = ath;
		if (ep == 2) ath --;
		if (ep == 3) ath -= rand() % 2 + 1;
		if (ep == 4) ath -= rand() % 3 + 1;
		if (ep == 5) ath -= rand() % 6 + 2;
		tp--;
		alnc = 0;
		tc = 0;
	}
...
int main()
{
	...
	do
	{
		...
		if (ath > 80)
		{
			...
			if (tl < pr)
			{
				printf("\nToo much authority with current technology");
				ath -= 3;
			}
		}
		if (tp > 3)
